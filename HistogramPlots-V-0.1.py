...
main_root_dir = r"C:\Users\syagi\Documents\PH007"    # change as per your folder name

# Define a single PDF filename to store all histogram plots and images
pdf_filepath = os.path.join(main_root_dir, "PH007-4X-CH3.pdf") # Name of the Pdf file in double quotes " " 
...
with PdfPages(pdf_filepath) as pdf:
    ...
    for exposure_folder in sorted(os.listdir(main_root_dir)):  # Sort for consistency
        ...
        # Save TIFF images in 4 columns per folder
        if tiff_images:
            # Number of columns (4)
            cols = 4
            rows = (len(tiff_images) + cols - 1) // cols  # Calculate number of rows needed

            fig, axes = plt.subplots(rows, cols, figsize=(cols * 3, rows * 3))  # Set figure size based on rows and columns
            axes = np.array(axes).flatten()  # Flatten to handle 1D indexing

            for i, (tiff_file, img_array) in enumerate(tiff_images):
                ax = axes[i]
                ax.imshow(img_array, cmap='gray')
                ax.axis('off')  # Hide axes
                ax.set_title(f"{tiff_file}", fontsize=8)

            # Hide empty subplots if there are fewer images than the total slots in the grid
            for j in range(i + 1, len(axes)):
                axes[j].axis("off")

            plt.tight_layout()
            pdf.savefig(fig)  # Save the figure with images to PDF
